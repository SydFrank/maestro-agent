...
class Settings(BaseServiceSettings):
    ...
    max_tool_iterations: int = 8  # ReAct loop safety bound (single worker)
    # Coding fix loops need more rounds (coder→tester→fixer→tester→reviewer...).
    max_supervisor_rounds: int = 8  # multi-agent loop safety bound
